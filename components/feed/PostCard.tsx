    ...
    <article className="surface-panel card-lift overflow-hidden p-4 sm:p-5">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="truncate text-sm font-semibold text-text-primary">{post.author.fullName}</div>
            ...
            {topRep ? (
              <div className="hidden shrink-0 sm:block">
                <ReputationBadge score={topRep.score} communityName={topRep.communityName} compact />
              </div>
            ) : null}
          </div>
      ...
      <div className="mt-4 space-y-3">
        <div className="flex flex-wrap items-start gap-2">
          ...
          {post.startupIdea ? <ValidationScoreBadge score={post.startupIdea.validationScore} compact /> : null}
        </div>

        <p className="line-clamp-3 text-sm leading-6 text-text-secondary">{post.body}</p>
        ...
        {post.startupIdea ? (
          <div className="flex flex-wrap gap-2 text-xs text-text-secondary">
            ...
            <Badge variant="secondary">{post.startupIdea.uniqueCommenters} unique voices</Badge>
          </div>
        ) : null}

        {!topRep ? (
          <div className="text-xs text-text-tertiary sm:hidden">Reputation grows with useful answers and votes.</div>
        ) : (
