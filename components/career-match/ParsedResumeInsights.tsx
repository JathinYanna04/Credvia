...
export function ParsedResumeInsights({ detail }: ParsedResumeInsightsProps) {
  ...
  const isNoiseLine = (line: string) => {
    ...
    const internalHits =
      (line.match(/\b(xref|obj|stream|endstream|startxref|flatedecode)\b/gi)?.length ??
        0) + slashHits;
  ...
  const sanitizeLines = (items: string[]) => items.filter((item) => !isNoiseLine(item));
  const summaryText = profile?.summary ?? null;
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div>
                ...
                <ul className="mt-2 space-y-2 text-sm text-text-secondary">
                  {sanitizedExperience.length > 0 ? sanitizedExperience.slice(0, 4).map((item) => <li key={item}>{item}</li>) : <li>No structured experience lines yet.</li>}
                </ul>
              </div>
              <div>
                <div className="text-xs uppercase tracking-[0.16em] text-text-tertiary">Projects</div>
                <ul className="mt-2 space-y-2 text-sm text-text-secondary">
                  {sanitizedProjects.length > 0 ? sanitizedProjects.slice(0, 4).map((item) => <li key={item}>{item}</li>) : <li>No structured project lines yet.</li>}
                </ul>
              </div>
              <div>
                <div className="text-xs uppercase tracking-[0.16em] text-text-tertiary">Education</div>
                <ul className="mt-2 space-y-2 text-sm text-text-secondary">
                  {sanitizedEducation.length > 0 ? sanitizedEducation.slice(0, 4).map((item) => <li key={item}>{item}</li>) : <li>No structured education lines yet.</li>}
                </ul>
              </div>
            </div>
          </div>
