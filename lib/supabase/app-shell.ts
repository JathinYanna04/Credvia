...
export const getAppShellData = cache(
  async function getAppShellData(): Promise<AppShellData> {
    ...
    const [profileResult, membershipsResult, unreadResult, reputationResult] =
      await Promise.all([
        supabase
          .from("profiles")
          .select("*")
    ...
    const reputationCommunityIds = (reputationResult.data ?? []).map(
    ...
    const communitiesMap = new Map(
      (communitiesForReputation.data ?? []).map((community) => [
        community.id,
        community,
      ]),
    );
    ...
    const joinedCommunities = (
      ...
      .filter((community): community is JoinedCommunityRow =>
        Boolean(community),
      )
      .map((community) => ({
        id: community.id,
