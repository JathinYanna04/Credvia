...
export async function GET(request: Request) {
  try {
    ...
    const membershipIds = (membershipsResult.data ?? []).map((item) => item.community_id);
    const followedIds = (followsResult.data ?? []).map((item) => item.followed_id);
    ...
    const summaries = await toPostSummaries(supabase, filteredPosts);
    const ranked =
      tab === 'for-you' ? getRankedFeed(summaries) : summaries;
