  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-10 sm:px-6">
      <header className="space-y-2">
        <h1 className="text-3xl font-semibold">Startup jobs</h1>
        <p className="max-w-3xl text-sm text-text-secondary">
          Browse live startup roles ingested into Credvia Career Match. No mock cards, only active jobs from the current source.
        </p>
      </header>
      ...
      <section className="space-y-4">
        ...
        {jobs?.map((job) => <JobBrowseCard key={job.id} job={job} />)}
      </section>
    </div>
  );
}
